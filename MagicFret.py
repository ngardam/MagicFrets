...
# */"""
# Credit to Aaron C Spike for his javascript solution
# Rewritten in RhinoPython in 2022 by Nicholas Gardam
...
def ff():
    ...
    def roundFloat(fltValue, intDecimal):
        return round(fltValue, intDecimal)
    ...
    class Point:
        ...
        def __str__(self):
            return ("(" + str(round(self.x, precision)) + "," + str(round(self.y, precision)) + ")")
    ...
    # };
        ...
        def midway(self, point):
            new_x = (point.x + self.x) * 0.5
            new_y = (point.y + self.y) * 0.5
    ...
    # }
        ...
        def run(self):
            return self.deltaX(self)

        def deltaY(self):
            return self.end2.y - self.end1.y
        def rise(self):
            return self.deltaY(self)
    ...
    # };
        ...
        def intercept(self):
            if self.deltaX() is not 0:
                return self.end2.y - (self.end2.x * self.slope())
    ...
    # };
        ...
        def length(self):
            return pow(pow(self.deltaX(), 2) + pow(self.deltaY(), 2), 0.5)

        def distanceToPoint(self, point):
            if self.length() is not 0:
                a = self.deltaX() * (self.end1.y - point.y)
                b = (self.end1.x - point.x) * self.deltaY()
                return abs((a - b)/self.length())

        def angle(self):
            return atan2(self.deltaY(), self.deltaX()) * (180 / pi)
